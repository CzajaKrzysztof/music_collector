...
def show_table(albums_list, table_title, sugestion='no'):
    if albums_list == None:
        ...
        print("{:^{width_lp}} | {:^{width_artist}s} | {:^{width_title}s} | {:^{width_year}} | {:^{width_genre}s} |
                {:^{width_time}s} |".format('lp.', 'Artist', 'Title', 'Year', 'Genre', 'Length', width_lp=lp_length,
                width_artist=artis_length, width_title=title_length, width_year=year_length, width_genre=genre_length, width_time=time_length))

        print('-' * (lp_length + artis_length + title_length + year_length + genre_length + time_length + 17))
        for item in albums_list:
            print("{:^{width_lp}} | {:^{width_artist}s} | {:^{width_title}s} | {:^{width_year}} | {:^{width_genre}s} |
                    {:^{width_time}s} |".format(i, item[0], item[1], item[2], item[3], item[4], width_lp=lp_length,
                    width_artist=artis_length, width_title=title_length, width_year=year_length, width_genre=genre_length, width_time=time_length))
            i += 1
